...
// CORRECCIÓN: Importamos desde 'catalog', no 'movies'
import { movies } from "@/data/movies/catalog"; 
import { Button } from "@/components/ui/button";

export const Hero = () => {
  // FILTRO: Solo mostramos películas que tengan imagen de fondo (backdrop) definida.
  // Si quieres que salgan más, asegúrate de añadir la propiedad 'backdrop' en catalog.ts
  const featuredMovies = movies.filter((movie) => movie.backdrop && movie.backdrop !== "");
  ...
  useEffect(() => {
    if (featuredMovies.length <= 1) return; // No animar si solo hay 1 peli

    const timer = setInterval(() => {
      handleNext();
    }, 6000);
    return () => clearInterval(timer);
  }, [currentSlide, featuredMovies.length]);
  ...
  // Si no hay ninguna película con backdrop, no mostramos nada (o podrías mostrar un banner por defecto)
  ...
  return (
    <section className="relative w-full h-[85vh] md:h-[95vh] overflow-hidden bg-black">
      ...
      <div 
        key={movie.id} // Usamos movie.id para asegurar que React detecte el cambio
      ...
      {/* INDICADORES (PUNTITOS) - Solo se muestran si hay más de 1 película */}
